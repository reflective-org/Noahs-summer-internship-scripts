...
def load_measured_distribution(
    csv_path: str,
    dp_units: str = 'nm',
) -> tuple[np.ndarray, np.ndarray]:
    """Load a measured dN/dlog10Dp size distribution from a CSV file.

    Expected format: two columns, [Dp, dN/dlog10Dp], with Dp the particle
    diameter (units set by `dp_units`) and dN/dlog10Dp the number
    concentration density [#/cm3]. An optional header row and blank/comment
    lines are skipped automatically.

    Args:
        csv_path: Path to the CSV file.
        dp_units: Diameter units in the file ('nm', 'um'/'µm', or 'm').

    Returns:
        (Dp_m, dNdlog10Dp): diameter [m] and dN/dlog10Dp [#/cm3], sorted by
        ascending diameter.
    """
    import csv as _csv

    scale = {
        'nm': 1e-9, 'um': 1e-6, 'µm': 1e-6, 'micron': 1e-6, 'm': 1.0,
    }
    key = dp_units.lower()
    if key not in scale:
        raise ValueError(f"Unknown dp_units '{dp_units}'. Use one of {list(scale)}.")
    ...
    with open(csv_path, newline='') as f:
        for row in _csv.reader(f):
            ...
            try:
                ...
                continue  # header / comment / malformed line
    if not rows:
        raise ValueError(f"No numeric (Dp, dN/dlog10Dp) rows found in {csv_path}")

    arr = np.asarray(rows, dtype=np.float64)
    Dp_m = arr[:, 0] * scale[key]
    dNdlog10Dp = arr[:, 1]
    order = np.argsort(Dp_m)
    return Dp_m[order], dNdlog10Dp[order]


def measured_to_Nk_Mk(
    Dp_m: np.ndarray,
    dNdlog10Dp: np.ndarray,
    xk: jnp.ndarray,
    density: float = 1770.0,
    n_subsample: int = 50,
) -> tuple[jnp.ndarray, jnp.ndarray]:
    """Bin a measured dN/dlog10Dp distribution onto the TOMAS mass grid.

    Each TOMAS mass bin [xk[k], xk[k+1]] is converted to a diameter range
    using the same density used to build the grid, then the measured
    dN/dlog10Dp curve is integrated over that bin's log10(Dp) width to give
    the bin number Nk. Bin mass is assigned as Nk * (geometric-mean bin mass)
    into SO4, mirroring `create_lognormal_dist`.

    Args:
        Dp_m: Measured diameters [m] (ascending).
        dNdlog10Dp: Measured dN/dlog10Dp [#/cm3].
        xk: Mass bin boundaries [kg], shape (nbins+1,) — the canonical grid.
        density: Particle density [kg/m3] for mass<->diameter (matches grid).
        n_subsample: Integration points per bin (trapezoid in log10(Dp)).

    Returns:
        (Nk, Mk): number [#/cm3] shape (nbins,) and mass [kg/cm3] shape
        (nbins, ICOMP). Multiply by boxvol for per-grid-cell quantities.
    """
    _trapz = np.trapezoid if hasattr(np, 'trapezoid') else np.trapz

    xk_np = np.asarray(xk, dtype=np.float64)
    nbins = xk_np.shape[0] - 1

    # Mass boundaries -> diameter boundaries (m = pi/6 * rho * D^3)
    Dp_edges = np.cbrt(xk_np / density * (6.0 / np.pi))
    log10_edges = np.log10(Dp_edges)
    log10_meas = np.log10(Dp_m)

    # Integrate measured dN/dlog10Dp over each bin's log10(Dp) interval.
    # `np.interp` with left/right=0 zeroes out bins outside the measured range.
    Nk = np.zeros(nbins, dtype=np.float64)
    for k in range(nbins):
        L = np.linspace(log10_edges[k], log10_edges[k + 1], n_subsample)
        f = np.interp(L, log10_meas, dNdlog10Dp, left=0.0, right=0.0)
        Nk[k] = _trapz(f, L)

    # Bin mass: number x geometric-mean bin mass, placed in SO4
    m_mid = np.sqrt(xk_np[:-1] * xk_np[1:])
    Mk = np.zeros((nbins, ICOMP), dtype=np.float64)
    Mk[:, SRTSO4] = Nk * m_mid

    return jnp.asarray(Nk), jnp.asarray(Mk)
...
def run_box_model(enable_condensation: bool = True, method: str = 'ppm_jit',
                   ...
                   measured_csv: str = None, dp_units: str = 'nm',
                   temp: float = 298.0, pres: float = 101325.0,
                   run_days: float = 1.0, dt: float = 60.0):
    ...
    temp = float(temp)  # Kelvin (caller-specified)
    pres = float(pres)  # Pascals (caller-specified)
    rh = 0.5        # 50% relative humidity
    alpha = 1.0     # Accommodation coefficient
    print(f"   Temp/Pres:    {temp:.1f} K / {pres:.1f} Pa")

    # --- B. Initialize Particles ---
    if measured_csv is not None:
        # Measured dN/dlog10Dp distribution on the canonical TOMAS mass grid
        xk = config.xk_boundaries()
        Dp_m, dNdlog10Dp = load_measured_distribution(measured_csv, dp_units)
        Nk_init, Mk_init = measured_to_Nk_Mk(Dp_m, dNdlog10Dp, xk)
        print(f"\n[Init] Measured distribution: {measured_csv}")
        print(f"       {len(Dp_m)} points, "
              f"Dp = {Dp_m.min()*1e9:.2f}-{Dp_m.max()*1e9:.2f} nm ({dp_units})")
        print(f"       Binned onto {NBINS}-bin canonical grid "
              f"(N_total = {float(jnp.sum(Nk_init)):.4e} #/cm3)")
    else:
    ...
    # H2SO4 gas source: only meaningful when a consumer (condensation or
    # nucleation) is active. For coagulation-only runs (both off) there is no
    # gas-phase interaction — coagulation never touches Gc — so the source is
    # zeroed to keep gas diagnostics clean. This applies equally to plain
    # `--no-nucleation --no-condensation` and to measured-distribution runs.
    if enable_condensation or enable_nucleation:
        h2so4_init_molec_cm3 = 1.0e7          # typical atmospheric
        h2so4_prod_rate_molec_cm3_s = 1.0e7   # typical polluted BL
    else:
        h2so4_init_molec_cm3 = 0.0
        h2so4_prod_rate_molec_cm3_s = 0.0
    ...
    # --- E. Time Loop ---
    total_time = 86400.0 * run_days   # run length [s] (caller-specified, default 1 day)
    dt_model = float(dt)              # timestep [s] (caller-specified, default 60 s)
...
if __name__ == "__main__":
    ...
    # Measured-distribution coagulation-only initialization
    parser.add_argument('--measured-csv', type=str, default=None,
                        help='Path to measured dN/dlog10Dp CSV [Dp, dN/dlog10Dp]. '
                             'Initializes the size distribution on the canonical '
                             'mass grid; combine with the process flags (e.g. '
                             '--no-nucleation --no-condensation for coag only). '
                             'The values are taken as AMBIENT number density at '
                             '--temp/--pres and as DRY diameters: no STP->ambient '
                             'conversion is applied here. Instrument files often '
                             'report at STP (e.g. SABRE AMP at 273.15 K/1013 hPa, '
                             'a 5.7x factor in the lower stratosphere) - convert '
                             'first, or use experimental_case/run_sabre_coag.py, '
                             'which does it for you.')
    parser.add_argument('--dp-units', choices=['nm', 'um', 'm'], default='nm',
                        help='Diameter units in the measured CSV (default: nm)')
    parser.add_argument('--temp', type=float, default=298.0,
                        help='Initial temperature [K] (default: 298)')
    parser.add_argument('--pres', type=float, default=101325.0,
                        help='Initial pressure [Pa] (default: 101325)')
    parser.add_argument('--run-days', type=float, default=1.0,
                        help='Simulation run length [days] (default: 1)')
    parser.add_argument('--dt', type=float, default=60.0,
                        help='Model timestep [seconds] (default: 60)')
