...
# =========================================================================
# 3b. OH parabola in solar zenith angle (Hanisco et al., 2001, Fig. 1)
# =========================================================================

@jax.jit
def calc_oh_parabola(cos_sza, oh_peak):
    """OH concentration from a parabolic fit in solar zenith angle.

    Fit to Hanisco et al. (2001), Fig. 1: OH rises from 0 at the horizon
    (SZA = 90 deg) to its peak at overhead sun (SZA = 0 deg), following a
    downward parabola in the (unsigned) zenith angle:

        OH(theta) = oh_peak * max(0, 1 - (theta / 90)^2)

    with theta = arccos(cos_sza) in degrees. Using the unsigned angle makes
    the morning and afternoon branches symmetric; the max(., 0) clamp sends
    OH to zero for SZA >= 90 deg (sun at/below the horizon -> night), so OH is
    never negative.

    Args:
        cos_sza: Cosine of solar zenith angle (from calc_solar_zenith_angle);
                 may be negative at night.
        oh_peak: Peak OH concentration at SZA = 0 [molec/cm^3].

    Returns:
        oh: OH concentration [molec/cm^3], zero at night.
    """
    # unsigned zenith angle [deg]; clip into [-1, 1] so arccos is always valid
    theta_deg = jnp.arccos(jnp.clip(cos_sza, -1.0, 1.0)) * (180.0 / jnp.pi)
    frac = 1.0 - (theta_deg / 90.0) ** 2
    return oh_peak * jnp.maximum(0.0, frac)
